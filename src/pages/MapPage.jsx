...
const MapPage = () => {
  ...
  const addMarkersToMap = (reportsData) => {
    ...
    // Clear only our custom data sources and layers (not built-in ones)
    try {
      // Remove our custom data source if it exists
      if (map.sources.getById('reportsDataSource')) {
        map.sources.remove('reportsDataSource');
      }
      
      // Remove our custom layers if they exist
      ['bubble-layer', 'symbol-layer'].forEach(layerId => {
        if (map.layers.getById(layerId)) {
          map.layers.remove(layerId);
        }
      });
    } catch (error) {
      console.log('Clearing previous layers:', error);
    }

    // Create data source with a specific ID
    const dataSource = new atlas.source.DataSource('reportsDataSource');
    map.sources.add(dataSource);

    // Add points to data source with proper coordinates
    ...
    const bubbleLayer = new atlas.layer.BubbleLayer(dataSource, 'bubble-layer', {
      radius: 12,
      strokeWidth: 3,
      strokeColor: 'white',
      color: [
    ...
    // Add symbol layer for labels with ID
    const symbolLayer = new atlas.layer.SymbolLayer(dataSource, 'symbol-layer', {
      textOptions: {
        textField: ['get', 'title'],
        color: 'white',
        haloColor: 'black',
